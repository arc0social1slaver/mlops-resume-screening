...
def model_evaluation(X_test, y_test, processor: ProcessingData) -> None:
    ...
    with mlflow.start_run() as run:
        try:
            ...
            ip_example = pd.DataFrame(
                X_test[:5], columns=processor.tfidf.get_feature_names_out()
            )
            sign = infer_signature(ip_example, model.predict(X_test[:5]))
            mlf_sklearn.log_model(
                model, "KNeigbours", signature=sign, input_example=ip_example
            )
            ...
            with open(
                ...
                model_info = {"model_path": "KNeigbours", "run_id": run.info.run_id}
            ...
            classification_rp = classification_report(
...
if __name__ == "__main__":
    test_data = load_data(
        data_url=os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../../data/processed/test.csv"
        )
    )
